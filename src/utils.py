...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
                          device_map="auto", device="cuda", use_flash_attn=False, dtype=None, **kwargs):
    ...
    if load_8bit:
        ...
        kwargs['dtype'] = dtype if dtype is not None else torch.bfloat16
    ...
    if is_lora_model(model_path) and model_base is not None:
        ...
        processor = AutoProcessor.from_pretrained(model_base, fix_mistral_regex=True)
        ...
        if lora_cfg_pretrained.model_type == "qwen3_vl_moe":
        ...
        elif lora_cfg_pretrained.model_type == "qwen2_vl":
            ...
            raise ValueError(f"Unsupported model type: {lora_cfg_pretrained.model_type}")

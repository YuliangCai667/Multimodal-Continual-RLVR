...
The two-step run above is an implementation preflight, not a shortened performance experiment. `EXP-CTIR-004` is now queued in a live detached watcher for a four-GPU 300-step Puzzle stage from the exact T3-final weights, with seed 42 and the existing formal GRPO configuration. Runtime logs, checkpoints, and model weights are local artifacts and must not be pushed to GitHub. Poll-and-launch minimizes the scheduling window but cannot provide exclusive ownership against another user launching simultaneously; only a cluster scheduler or administrator-enforced exclusive mode can provide that guarantee.

## Publication boundary

Publish as one new-history initial commit to the private repository `YuliangCai667/Multimodal-Continual-RLVR`. Do not push to the original `MaolinLuo/CPO` remote. Include source, launch/configuration code, focused tests, and concise engineering/research records; exclude checkpoints, raw evaluations, generated figures, logs, sessions, and credentials. The first snapshot was published on 2026-09-03; the final numerical correction and this evidence are folded into that same root commit rather than adding upstream CPO history.

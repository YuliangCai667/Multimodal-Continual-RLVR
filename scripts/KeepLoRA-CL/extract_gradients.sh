...
if [ $TASK_ID -eq 1 ]; then
    ...
    SPACE_PATH=./checkpoints/Qwen3-VL-2B/KeepLoRA-CL/lora_gradients/Init
else
    PREV_DATASET=${DATASETS[$((TASK_ID - 2))]}
    MODEL_NAME=./checkpoints/Qwen3-VL-2B/KeepLoRA-CL/model/${PREV_DATASET}
    SPACE_PATH=./checkpoints/Qwen3-VL-2B/KeepLoRA-CL/lora_gradients/${PREV_DATASET}
fi

if [ "${SVD_MODE}" = "energy" ]; then
    MODEL_NAME=./checkpoints/Qwen3-VL-2B/KeepLoRA-CL/model/${CUR_DATASET}
fi

DATA_PATH=${BASE_PATH}/${CUR_DATASET}/jsons/train/data.json
MEDIA_DIR=${BASE_PATH}/${CUR_DATASET}/images
OUTPUT_DIR=./checkpoints/Qwen3-VL-2B/KeepLoRA-CL/lora_gradients/${CUR_DATASET}
RANK=128

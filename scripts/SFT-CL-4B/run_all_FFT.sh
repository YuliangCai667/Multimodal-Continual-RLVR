...
MAIN_LOG=./checkpoints/Qwen3-VL-4B/FFT-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-4B/FFT-CL
...
do
    ...
    bash scripts/SFT-CL-4B/sft_FFT.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/SFT-CL-4B/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done

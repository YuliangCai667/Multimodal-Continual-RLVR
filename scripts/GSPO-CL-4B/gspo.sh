...
BATCH_PER_DEVICE=8   # https://github.com/huggingface/trl/issues/3061
MAX_STEPS=600
LR=5e-6
...
if [ $TASK_ID -eq 1 ]; then
    ...
    MODEL_NAME=./checkpoints/Qwen3-VL-4B/GSPO-CL/training/${PREV_DATASET}
fi

MASK_ARGS=""
if [ $TASK_ID -gt 1 ]; then
    MASK_PATH=./checkpoints/Qwen3-VL-4B/GSPO-CL/masks/task_$((TASK_ID - 1)).pt
    if [ -f "$MASK_PATH" ]; then
...
OUTPUT_DIR=./checkpoints/Qwen3-VL-4B/GSPO-CL/training/${CUR_DATASET}
MERGE_SAVE_PATH=./checkpoints/Qwen3-VL-4B/GSPO-CL/model/${CUR_DATASET}

...
MAIN_LOG=./checkpoints/Qwen3-VL-8B/FFT-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-8B/FFT-CL
...
do
    ...
    bash scripts/SFT-CL-8B/sft_FFT.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/SFT-CL-8B/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done

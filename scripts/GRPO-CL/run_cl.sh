...
MAIN_LOG=./checkpoints/Qwen3-VL-2B/GRPO-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-2B/GRPO-CL
...
do
    ...
    bash scripts/GRPO-CL/grpo.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/GRPO-CL/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done

...
MAIN_LOG=./checkpoints/Qwen3-VL-2B/LoRA-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-2B/LoRA-CL
...
do
    ...
    bash scripts/LoRA-CL/sft_lora.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/LoRA-CL/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done

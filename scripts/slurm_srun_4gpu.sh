...
CPUS_PER_TASK=${MRCL_CPUS_PER_TASK:-14}
...
if [ "$WORLD_SIZE" -ne 4 ]; then
    echo "EXP-CTIR-MULTI-T1-T5-001 requires exactly 4 global GPU ranks, got $WORLD_SIZE" >&2
    exit 1
fi

export MASTER_ADDR=${MASTER_ADDR:-${ALLOCATED_NODES[0]}}
export MASTER_PORT=${MASTER_PORT:-$((20000 + SLURM_JOB_ID % 20000))}
echo "Launching four one-GPU ranks across $NODE_COUNT node(s): ${ALLOCATED_NODES[*]}"

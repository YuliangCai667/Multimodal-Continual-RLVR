...
MAIN_LOG=./checkpoints/Qwen3-VL-8B/GRPO-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-8B/GRPO-CL
...
do
    ...
    bash scripts/GRPO-CL-8B/grpo.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/GRPO-CL-8B/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done

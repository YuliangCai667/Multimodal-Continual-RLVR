...
MAIN_LOG=./checkpoints/Qwen3-VL-2B/OLoRA-CL/training.log
mkdir -p ./checkpoints/Qwen3-VL-2B/OLoRA-CL
...
do
    ...
    bash scripts/OLoRA-CL/sft_olora.sh "$BASE_MODEL" "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
    conda activate vllmQwen
    bash scripts/OLoRA-CL/eval.sh "$BASE_PATH" "$TASK_ID" "${DATASETS[@]}"
done
